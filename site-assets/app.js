(() => {
  ...
  if (finePointer && desktopPointer && !reduceMotion && cursorDot && cursorRing) {
    ...
    document.querySelectorAll("a, button, input, select, textarea, .service-card, .work-card, .card, .result-card, .process-card, .overview-card, .link-card, .faq-item, .image-placeholder, .image-frame, .client-card, .proof-quote, .case-preview, .team-role-grid article, .brand-cloud span").forEach((item) => {
